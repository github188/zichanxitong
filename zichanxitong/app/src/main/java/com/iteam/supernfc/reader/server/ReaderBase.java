	...
	private class WaitThread extends Thread {
		...
		public void run() {
			...
			while (mShouldRunning) {
				try {
					//Loger.disk_log("调试日志", "read前——开始接受数据", "M10_U8");
					...
					Loger.disk_log("读错误", "e="+e.getMessage()+"\r\n"+e.getStackTrace(), "M10_U8");
	...
	private int sendMessage(byte[] btArySenderData) {
		...
		try {
			synchronized (mOutStream) { // 防止并发
				...
				//Loger.disk_log("新增日志", "write后"+StringTool.byteArrayToString(btArySenderData, 0, btArySenderData.length), "M10_U8");
			}
		} catch (IOException e) {
